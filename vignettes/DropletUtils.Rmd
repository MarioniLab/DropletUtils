...
The function will also adjust for differences in the ambient levels of each HTO based on the ambient profile;
if this is not provided, it will instead attempt to estimate the ambient profile from the supplied count matrix.
...
It is then a simple matter to assign each cell back to its sample of origin.
We provide `Confident` calls to indicate which cells are confident singlets,
based on the fact that they are (i) not doublets and (ii) do not have small log-fold changes between the top and second HTO.
...
We also identify doublets based on the log-fold change between the second and third-most abundant HTO.
Here, the assumption is that counts for the third or later HTOs are caused by ambient contamination.
A large log-fold change indicates that the second HTO exceeds that from contamination, 
consistent with the presence of a doublet.

```{r}
colors <- ifelse(demux$Confident, "black",
    ifelse(demux$Doublet, "red", "grey"))
plot(demux$LogFC.1to2, demux$LogFC.2to3, col=colors)
```
