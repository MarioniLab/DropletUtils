...
Given a count matrix of all barcodes (e.g., from `raw_gene_bc_matrices`, for 10X data), we call:
...
Droplets with significant deviations from the ambient profile are detected at a specified FDR threshold, e.g., with `FDR` below 5%.
...
```{r}
table(Limited=e.out$Limited, Significant=e.out$FDR <= 0.05)
```

We also recommend making some diagnostic plots, such as the total count against the deviance.
...
```{r}
plot(e.out$Total, e.out$Deviance, xlab="Total count", ylab="Deviance",
    col=ifelse(e.out$FDR <= 0.05, "red", "black"), pch=16)
o <- order(e.out$Total)
