...
We can load this into memory using the `read10xCounts` function:

```{r, echo=FALSE}
set.seed(1000)
example(read10xCounts, echo=FALSE) # to generate my files.
dir.name <- tmpdir
```
...
Users can also load multiple results in at once, by supplying a character vector to `read10xCounts`.
