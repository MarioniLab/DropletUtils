...
#'
#' We identity the sample of origin for each barcode based on the HTO with the greatest deviation from the expected proportions.
#' This is quantified by the ratio of the count for each HTO to the corresponding value of \code{prop}.
#' In this manner, we account for potential imbalances in the distribution of HTOs in the multiplexed sample.
#' The log-fold change in the largest and second-largest ratios is also reported for each barcode,
...
#'
#' For both log-fold changes, we compute the number of MADs from the median across all cells.
#' This allows users to easily identify doublets as large outliers in \code{NMAD.2to3}.
...
#' Log-fold changes for empty droplets will simply reflect stochasticity in the ambient solution,
